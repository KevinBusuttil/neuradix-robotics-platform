...
use neuradix_python::{HeartbeatPolicy, IoLimits, PythonWorker, Timeouts, WorkerConfig, WorkerError, WorkerFailure, WorkerSupervisor};
...
fn config(mode: &str) -> WorkerConfig {
    WorkerConfig::new("python3", format!("{}/tests/workers/heartbeat.py", env!("CARGO_MANIFEST_DIR")))
        .with_arg(mode)
        .with_limits(IoLimits::new(256, 256, 1024, 4).unwrap())
        .with_timeouts(Timeouts::new(Duration::from_secs(1), Duration::from_secs(1), Duration::from_millis(300), RESERVE).unwrap())
...
scenarios!(healthy_idle, sdk_idle, ordinary_requests, hung_handler, stopped_worker,
    scheduling_gap, late_poll, unrelated_traffic, duplicate_replies, late_reply,
    flood_bounds, replacement_state, failed_replacement_handshakes, failed_launches,
    shutdown_and_drop);

#[test]
fn heartbeat_child() {
    let Ok(case) = std::env::var("NEURADIX_HEARTBEAT_CASE") else { return; };
    ...
    match case.as_str() {
        ...
        "sdk_idle" => {
            let cfg = WorkerConfig::new("python3", format!("{}/tests/workers/testkit_worker.py", env!("CARGO_MANIFEST_DIR")))
            ...
            assert!(worker.check_heartbeat().unwrap());
            assert_eq!(worker.health(), HealthState::Healthy);
            assert_eq!(worker.send(&json!({"depth": 3})).unwrap()["echo"]["depth"], 3);
        }
        "ordinary_requests" => {
            ...
            assert!(matches!(worker.send(&json!("remote")), Err(WorkerError::Remote(_))));
            assert_eq!(worker.health(), HealthState::Healthy);
            let due = worker.heartbeat_due();
            assert!(due > before);
            assert!(!worker.check_heartbeat().unwrap());
            assert!(matches!(worker.send(&json!("x".repeat(300))), Err(WorkerError::OutgoingTooLarge { .. })));
        ...
        "hung_handler" => {
            ...
            assert!(matches!(worker.check_heartbeat(), Err(WorkerError::Unavailable)));
        }
        "stopped_worker" => {
            let mut worker = PythonWorker::launch(&config("healthy")).unwrap();
            nix::sys::signal::kill(nix::unistd::Pid::from_raw(worker.process_id() as i32), nix::sys::signal::Signal::SIGSTOP).unwrap();
            until(worker.heartbeat_due());
            let start = Instant::now();
        ...
        "scheduling_gap" => {
            ...
            let start = Instant::now();
            assert!(matches!(cause(&worker.check_heartbeat().unwrap_err()), WorkerError::HeartbeatExpired));
            bounded(start, RESERVE);
            assert_eq!(worker.last_failure(), Some(WorkerFailure::HeartbeatExpired));
            assert!(matches!(worker.send(&Value::Null), Err(WorkerError::Unavailable)));
            assert_eq!(worker.health(), HealthState::Unavailable);
        }
        "late_poll" => {
            let cfg = config("hang").with_heartbeat(HeartbeatPolicy::new(INTERVAL, Duration::from_millis(700)).unwrap());
            let mut worker = PythonWorker::launch(&cfg).unwrap();
            until(worker.heartbeat_due() + Duration::from_millis(500));
            let start = Instant::now();
            assert!(matches!(cause(&worker.check_heartbeat().unwrap_err()), WorkerError::HeartbeatTimeout));
        ...
        "unrelated_traffic" | "duplicate_replies" | "late_reply" | "flood_bounds" => {
            let mode = match case.as_str() { "unrelated_traffic" => "unrelated", "duplicate_replies" => "duplicate", "late_reply" => "late", _ => "flood" };
            let cfg = config(mode);
            let mut worker = PythonWorker::launch(&cfg).unwrap();
            if mode == "duplicate" {
                until(worker.heartbeat_due());
            ...
            let error = worker.check_heartbeat().unwrap_err();
            if mode == "flood" {
                assert!(matches!(cause(&error), WorkerError::QueueMessagesExceeded { .. } | WorkerError::QueueBytesExceeded { .. }));
            } else {
                assert!(matches!(cause(&error), WorkerError::HeartbeatTimeout));
            }
            bounded(start, RESPONSE + RESERVE);
        ...
        "replacement_state" | "failed_replacement_handshakes" => {
            let path = temp(&case);
            let mode = if case == "replacement_state" { "replace" } else { "failed_handshake" };
            let mut supervisor = WorkerSupervisor::start(config(mode).with_arg(path.to_string_lossy()), 2).unwrap();
            until(supervisor.worker().unwrap().heartbeat_due());
            assert!(matches!(cause(&supervisor.poll().unwrap_err()), WorkerError::HeartbeatTimeout));
            ...
            if mode == "replace" {
                ...
                assert_eq!(supervisor.last_failure(), Some(WorkerFailure::HeartbeatTimeout));
                assert_eq!(supervisor.worker().unwrap().send(&Value::Null).unwrap(), 1);
                assert_eq!(supervisor.health(), HealthState::Healthy);
            } else {
                for used in 1..=2 {
                    let start = Instant::now();
                    assert!(matches!(cause(&supervisor.poll().unwrap_err()), WorkerError::HandshakeTimeout));
        ...
        "failed_launches" => {
            ...
            let cfg = WorkerConfig::new(&path, format!("{}/tests/workers/heartbeat.py", env!("CARGO_MANIFEST_DIR")))
                .with_arg("hang").with_heartbeat(HeartbeatPolicy::new(INTERVAL, RESPONSE).unwrap());
        ...
        "shutdown_and_drop" => {
            ...
            assert!(matches!(supervisor.ensure_alive(), Err(WorkerError::Unavailable)));
