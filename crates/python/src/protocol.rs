...
impl Frames {
    pub fn new(limits: IoLimits) -> Self {
        Self {
            bytes: VecDeque::with_capacity(limits.queued_bytes()),
            lengths: VecDeque::with_capacity(limits.queued_messages()),
            partial: 0,
            limits,
            high_bytes: 0,
            high_messages: 0,
    ...
    pub fn push(&mut self, bytes: &[u8]) -> Result<(), WorkerError> {
        for &byte in bytes {
            ...
            {
                return Err(WorkerError::IncomingTooLarge {
                    limit: self.limits.incoming_bytes(),
                });
            }
            if self.bytes.len() == self.limits.queued_bytes() {
                return Err(WorkerError::QueueBytesExceeded {
                    limit: self.limits.queued_bytes(),
                });
            }
            if byte == b'\n' && self.lengths.len() == self.limits.queued_messages() {
                return Err(WorkerError::QueueMessagesExceeded {
                    limit: self.limits.queued_messages(),
                });
            }
...
impl Write for LimitedWriter {
    fn write(&mut self, bytes: &[u8]) -> io::Result<usize> {
        ...
        if self.failure.is_some() {
            return Err(io::Error::other("bounded serialization stopped"));
        }
...
fn check_value(
    ...
    if depth > 32 {
        return Err(WorkerError::Protocol(
            "outgoing JSON exceeds depth 32".into(),
        ));
    }
    if *remaining == 0 {
        return Err(WorkerError::Protocol(
            "outgoing JSON exceeds node budget".into(),
        ));
    }
    *remaining -= 1;
    match value {
        Value::Array(values) => {
            for v in values {
                check_value(v, depth + 1, remaining, deadline)?;
            }
        }
        Value::Object(values) => {
...
pub(crate) fn encode(
    ...
    let result = (|| -> Result<(), Box<dyn std::error::Error>> {
        if let Some(seq) = sequence {
            write!(writer, "{{\"kind\":\"request\",\"seq\":{seq},\"payload\":")?;
        }
...
mod tests {
    ...
    fn line_boundary_counts_newline_and_rejects_unterminated_growth() {
        ...
        assert!(matches!(
            frames.push(b"x"),
            Err(WorkerError::IncomingTooLarge { limit: 64 })
        ));
    ...
    fn complete_and_partial_lines_share_the_exact_byte_budget() {
        ...
        frames.push(b"\n").unwrap();
        assert_eq!(frames.pop().unwrap().len(), 51);
    }
    #[test]
    ...
    fn encoding_bounds_escaped_utf8_and_depth_before_writing() {
        ...
        assert!(matches!(
            encode(&value, Some(1), line.len() - 1, end),
    ...
    fn expired_serialization_deadline_is_never_extended() {
        assert!(matches!(
            encode(&Value::Null, Some(u64::MAX), 256, Instant::now()),
