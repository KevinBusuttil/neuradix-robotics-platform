...
impl IoLimits {
    ...
    pub fn new(incoming_bytes: usize, outgoing_bytes: usize, queued_bytes: usize, queued_messages: usize) -> Result<Self, WorkerError> {
        ...
        Ok(Self { incoming_bytes, outgoing_bytes, queued_bytes, queued_messages })
    }
    /// Maximum incoming JSON line bytes, including newline.
    pub fn incoming_bytes(self) -> usize { self.incoming_bytes }
    /// Maximum outgoing JSON line bytes, including newline.
    pub fn outgoing_bytes(self) -> usize { self.outgoing_bytes }
    ...
    pub fn queued_messages(self) -> usize { self.queued_messages }
    /// Exactly one synchronous request; concurrent access requires exclusive ownership.
    pub fn outstanding_requests(self) -> usize { 1 }
}
...
impl Timeouts {
    ...
    pub fn new(handshake: Duration, request: Duration, shutdown: Duration, cleanup: Duration) -> Result<Self, WorkerError> {
        if cleanup.is_zero() || cleanup > Duration::from_secs(1)
            || [handshake, request, shutdown].into_iter().any(|t| t <= cleanup || t > Duration::from_secs(60))
        {
            return Err(WorkerError::InvalidConfig("invalid total timeout or cleanup reserve"));
        }
        Ok(Self { handshake, request, shutdown, cleanup })
    }
    /// Total launch/handshake operation budget, including cleanup.
    pub fn handshake(self) -> Duration { self.handshake }
    /// Total request operation budget, including serialization and cleanup.
    pub fn request(self) -> Duration { self.request }
    /// Total explicit graceful-shutdown budget, including cleanup.
    pub fn shutdown(self) -> Duration { self.shutdown }
    /// Reserved cleanup budget; also the entire Drop budget.
    pub fn cleanup(self) -> Duration { self.cleanup }
}
impl Default for Timeouts {
    fn default() -> Self {
        Self::new(Duration::from_secs(10), Duration::from_secs(10), Duration::from_millis(500), Duration::from_millis(50)).expect("valid defaults")
    }
}
...
impl WorkerConfig {
    ...
    pub fn new(interpreter: impl Into<PathBuf>, script: impl Into<PathBuf>) -> Self {
        Self { interpreter: interpreter.into(), script: script.into(), args: Vec::new(), python_path: Vec::new(), config: Value::Null, skip_inputs: true, limits: IoLimits::default(), timeouts: Timeouts::default() }
    }
    /// Set trusted structured startup configuration; launch bounds its encoding.
    pub fn with_config(mut self, config: Value) -> Self { self.config = config; self }
    /// Add a Python import directory.
    pub fn with_python_path(mut self, dir: impl Into<PathBuf>) -> Self { self.python_path.push(dir.into()); self }
    ...
    pub fn with_skip_inputs(mut self, skip: bool) -> Self { self.skip_inputs = skip; self }
    /// Install validated immutable I/O limits.
    pub fn with_limits(mut self, limits: IoLimits) -> Self { self.limits = limits; self }
    /// Install validated immutable total timeouts.
    pub fn with_timeouts(mut self, timeouts: Timeouts) -> Self { self.timeouts = timeouts; self }
    ...
    pub fn with_request_timeout(mut self, timeout: Duration) -> Result<Self, WorkerError> {
        self.timeouts = Timeouts::new(self.timeouts.handshake, timeout, self.timeouts.shutdown, self.timeouts.cleanup)?;
        Ok(self)
    }
    /// Configured immutable I/O limits.
    pub fn limits(&self) -> IoLimits { self.limits }
    /// Configured immutable operation budgets.
    pub fn timeouts(&self) -> Timeouts { self.timeouts }
}
...
mod tests {
    ...
    fn invalid_storage_and_deadline_configurations_are_rejected() {
        for (incoming, outgoing, bytes, messages) in [(0,64,64,1), (64,usize::MAX,64,1), (128,64,127,1), (64,64,usize::MAX,1), (64,64,64,0), (64,64,64,129)] {
        ...
        assert!(WorkerConfig::new("python3","worker.py").with_request_timeout(Duration::ZERO).is_err());
