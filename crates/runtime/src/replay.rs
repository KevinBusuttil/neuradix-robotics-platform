...
use neuradix_time::{Clock, ManualClock, Timestamp};
...
impl ReplayLimits {
    ...
    pub fn new(
        ...
        if !(1..=1_000_000).contains(&steps)
            || !(1..=1_000_000).contains(&outputs)
            || !(1..=16 << 20).contains(&payload)
            || !(1024..=256 << 20).contains(&case_bytes)
            || diagnostics > 4096
            || report_bytes > 1 << 20
            || diagnostics
                .checked_mul(std::mem::size_of::<ReplayMismatch>())
                .is_none_or(|n| n > report_bytes)
        {
...
impl Default for ReplayLimits {
    fn default() -> Self {
        Self::new(10_000, 100_000, 1 << 20, 64 << 20, 64, 16 << 10).unwrap()
    }
}
...
impl ReplayCase {
    ...
    pub fn new(
        ...
        if configuration.len() > limits.payload || accounted > limits.case_bytes {
    ...
    pub fn push(
        ...
        if evaluation.domain() != self.start.domain() {
            return Err(ReplayError::ClockMismatch);
        }
        if evaluation.as_nanos() < previous.as_nanos() {
            return Err(ReplayError::ClockRegression);
        }
        evaluation
            .duration_since(previous)
            .map_err(|_| ReplayError::ClockOverflow)?;
        let count = add(self.expected_count, expected.len())?;
        if self.steps.len() >= self.limits.steps
            || count > self.limits.outputs
            || input.data.len() > self.limits.payload
        {
            return Err(ReplayError::Limit);
        }
        let mut charge = add(256, input.data.len())?;
        for value in expected {
            if value.len() > self.limits.payload {
                return Err(ReplayError::Limit);
            }
            charge = add(charge, add(64, value.len())?)?;
        }
        let accounted = add(self.accounted, charge)?;
        if accounted > self.limits.case_bytes {
            return Err(ReplayError::Limit);
        }
    ...
    /// Initial trusted replay evaluation time.
...
fn add(a: usize, b: usize) -> Result<usize, ReplayError> {
    a.checked_add(b).ok_or(ReplayError::Limit)
}
...
/// Bounded component error evidence; messages are untrusted diagnostics.
#[derive(Debug)]
...
impl ReplayFailure {
    fn from_error(error: ComponentError) -> Self {
        let (text, invalid_identity) = match error {
            ComponentError::InvalidId(s) => (s, true),
...
pub struct ReplayReport<'a> {
    ...
    /// Whether the selected factory returned an instance; isolation is its contract.
    pub initialized: bool,
    /// Fixed-size failure prefix; no unbounded error text is retained.
...
impl ReplayReport<'_> {
    ...
    pub fn omitted(&self) -> usize {
        self.differences.saturating_sub(self.diagnostics.len())
    }
}
...
pub fn run_replay<'a, P: ReplayProgram>(program: &P, case: &'a ReplayCase) -> ReplayReport<'a> {
    let mut report = ReplayReport {
        case,
        implementation: std::any::type_name::<P::Instance>(),
        outcome: ReplayOutcome::IdentityMismatch,
        initialized: false,
        failure: None,
        invoked: 0,
        completed: 0,
        outputs: 0,
        differences: 0,
        diagnostics: Vec::with_capacity(case.limits.diagnostics),
    };
    debug_assert!(
        report.diagnostics.capacity() * std::mem::size_of::<ReplayMismatch>()
            <= case.limits.report_bytes
    );
    if program.identity() != case.identity {
        return report;
    }
    let mut processor = match program.create(&case.configuration, case.seed) {
        Ok(processor) => processor,
        Err(error) => {
            report.outcome = ReplayOutcome::InitializationFailed;
            report.failure = Some(ReplayFailure::from_error(error));
            return report;
        }
    };
    ...
    for (index, step) in case.steps.iter().enumerate() {
        ...
        clock
            .set(step.evaluation)
            .expect("admitted replay clock domain");
        report.invoked += 1;
        let ctx = TickContext {
            now: clock.now(),
            sequence: index as u64,
        };
        let actual = match processor.process(&ctx, step.input.clone()) {
            Ok(actual) => actual,
            Err(error) => {
                report.outcome = ReplayOutcome::ProcessorFailed;
                report.failure = Some(ReplayFailure::from_error(error));
                return report;
            }
        };
        if actual.len() > case.limits.outputs - report.outputs
            || actual.iter().any(|v| v.len() > case.limits.payload)
        {
            report.outcome = ReplayOutcome::OutputLimit;
            return report;
        }
        report.outputs += actual.len();
        report.completed += 1;
        for output in 0..step.expected.len().max(actual.len()) {
            let expected = step.expected.get(output);
            let got = actual.get(output);
            if expected == got {
                continue;
            }
            report.differences += 1;
            if report.diagnostics.len() < case.limits.diagnostics {
                report.diagnostics.push(ReplayMismatch {
                    step: index,
                    output,
                    difference: match (expected, got) {
                        (None, _) => ReplayDifference::Extra,
                        (_, None) => ReplayDifference::Missing,
                        _ => ReplayDifference::Changed,
                    },
                    expected_bytes: expected.map(Vec::len),
                    actual_bytes: got.map(Vec::len),
                });
            }
        }
    }
    report.outcome = if report.differences == 0 {
