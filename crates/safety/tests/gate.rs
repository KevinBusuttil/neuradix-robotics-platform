...
use neuradix_runtime::{Processor, TickContext, run_lockstep};
use neuradix_safety::{
    AuthorityLease, Capability, CommandEnvelope, CommandMeta, CommandPolicy, CommandRequest,
    Constraint, Generation, Identity, LeaseTable, Outcome, RejectReason as R, SafetyDecision,
    SafetyError, SafetyGate, SessionConfig, SharedTimeline,
};
use neuradix_time::{ClockDomain, Duration, ManualClock, Timestamp};
fn t(n: i128) -> Timestamp {
    Timestamp::new(ClockDomain::Simulation, n)
}
...
fn evaluate(gate: &mut SafetyGate, value: f64, seq: u64, now: i128) -> SafetyDecision {
    gate.evaluate(Some(request(value, seq, now)), t(now))
}
...
fn no_lease_is_rejected_with_safe_output() {
    ...
    assert_eq!(d.outcome, Outcome::Rejected(R::UnknownBinding));
...
fn decisions_replay_identically_including_idle_expiry() {
    let inputs = vec![
        (t(0), Some(request(0.5, 0, 0))),
        (t(50_000_000), None),
        (t(100_000_000), None),
    ];
    let mut a = gate(vec![], None);
    let mut b = gate(vec![], None);
    let out = run_lockstep(&ManualClock::new(t(0)), &mut a, inputs.clone()).unwrap();
    assert_eq!(
        out,
        run_lockstep(&ManualClock::new(t(0)), &mut b, inputs).unwrap()
    );
...
fn non_finite_commands_fall_back_and_recovery_slews_from_safe() {
    for bad in [f64::NAN, f64::INFINITY, f64::NEG_INFINITY] {
        ...
        assert_eq!(d.outcome, Outcome::Rejected(R::NonFiniteCommand));
        assert_eq!(d.applied, 0.0);
...
fn constraint_order_and_extreme_finite_values_preserve_all_hard_bounds() {
    for reverse in [false, true] {
            ...
            .enumerate()
        {
            ...
            assert!(d.applied.is_finite() && (-0.5..=0.5).contains(&d.applied));
    ...
    for (first, elapsed) in [
        ...
        let mut g = gate(
            vec![
                ...
                Constraint::slew_rate("slew", f64::MAX).unwrap(),
            ],
            None,
        );
        assert_eq!(evaluate(&mut g, first, 0, 0).applied, first);
        let d = evaluate(&mut g, 0.0, 1, elapsed);
        assert_eq!(d.outcome, Outcome::Rejected(R::InvalidOutput));
        assert_eq!(d.applied, 0.0);
        assert_eq!(
            g.leases()
                .last_accepted_at(&Identity::new("controller"), &Capability::new("thrust")),
...
fn trusted_binding_table_is_bounded_and_retains_revoked_generation() {
    ...
    assert_eq!(
        table.grant(AuthorityLease::new(
            Identity::new("overflow"),
            Capability::new("c"),
            config(),
            None
        )),
        Err(neuradix_safety::SessionError::CapacityExceeded)
    );
    ...
    assert_eq!(
        ...
        Err(neuradix_safety::SessionError::ReusedGeneration)
    );
}
