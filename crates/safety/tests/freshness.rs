...
use neuradix_safety::{AuthorityLease, Capability, CommandMeta, CommandPolicy, CommandRequest, Constraint, Generation, Identity, LeaseTable, Outcome, RejectReason as R, SafetyGate, SessionConfig, SessionError, SharedTimeline};
...
fn config(generation: u128, age: i128, watchdog: i128) -> SessionConfig {
    SessionConfig::new(Generation::new(generation).unwrap(), t(0), t(1000), CommandPolicy::new(SharedTimeline::new(1, ClockDomain::Monotonic).unwrap(), Duration::from_nanos(age), Duration::from_nanos(10), Duration::from_nanos(watchdog)).unwrap()).unwrap()
}
fn meta(sequence: u64, source: i128, deadline: i128) -> CommandMeta {
    CommandMeta { generation: Generation::new(7).unwrap(), sequence, source_at: t(source), deadline: t(deadline), timeline: 1 }
}
struct Pair { host: SafetyGate, embedded: CommandGate }
impl Pair {
    fn new(config: SessionConfig) -> Self {
        let mut table = LeaseTable::new();
        table.grant(AuthorityLease::new(Identity::new("controller"), Capability::new("thrust"), config, None)).unwrap();
        Self {
            host: SafetyGate::new(table, vec![Constraint::range("range", -1.0, 1.0).unwrap()], 0.0).unwrap(),
            embedded: CommandGate::new(Limits::new(-1.0, 1.0, 2.0).unwrap(), mcu::AuthorityLease::new(1, 2, config), 0.0).unwrap(),
        }
    }
    fn step(&mut self, input: Option<(CommandMeta, f32, bool)>, now: Timestamp, expected: Option<R>) {
        let host = input.map(|(meta, value, known)| CommandRequest::new(Identity::new(if known { "controller" } else { "stranger" }), Capability::new("thrust"), value as f64, meta));
        let embedded = input.map(|(meta, value, known)| mcu::Command { holder: if known { 1 } else { 9 }, capability: 2, value, meta });
        let h = self.host.evaluate(host, now);
        let e = self.embedded.evaluate(embedded, now);
        let hr = match h.outcome { Outcome::Rejected(reason) => Some(reason), _ => None };
        let er = match e.outcome { mcu::Outcome::SafeState(reason) => Some(reason), _ => None };
        ...
        if expected.is_some() { assert_eq!(h.applied, 0.0); }
    }
    fn send(&mut self, meta: CommandMeta, now: i128, expected: Option<R>) { self.step(Some((meta, 0.5, true)), t(now), expected); }
    fn replace(&mut self, config: SessionConfig) -> Result<(), SessionError> {
        let h = self.host.leases_mut().grant(AuthorityLease::new(Identity::new("controller"), Capability::new("thrust"), config, None));
        let e = self.embedded.replace_lease(mcu::AuthorityLease::new(1, 2, config));
        assert_eq!(h, e);
        h
    }
    fn renew(&mut self, expires: i128, now: i128) -> Result<(), SessionError> {
        let h = self.host.leases_mut().renew(&Identity::new("controller"), &Capability::new("thrust"), t(expires), t(now));
        let e = self.embedded.renew_lease(t(expires), t(now));
        assert_eq!(h, e);
        h
    }
    fn accepted_at(&self, expected: Option<Timestamp>) {
        assert_eq!(self.host.leases().last_accepted_at(&Identity::new("controller"), &Capability::new("thrust")), expected);
...
fn rejected_packets_do_not_feed_watchdog_or_consume_sequence() {
    ...
    p.step(Some((meta(2, 152, 900), f32::NAN, true)), t(152), Some(R::NonFiniteCommand));
    p.accepted_at(Some(t(100)));
...
fn all_temporally_rejected_packets_leave_liveness_unchanged() {
    for expected in [R::StaleCommand, R::FutureCommand, R::DeadlineExpired, R::GenerationMismatch, R::UnsupportedClockRelationship, R::UnknownBinding] {
        let mut p = Pair::new(config(7, 100, 50));
        ...
        match expected {
            ...
            R::DeadlineExpired => { m.source_at = t(100); m.deadline = t(140); }
            R::GenerationMismatch => m.generation = Generation::new(8).unwrap(),
            R::UnsupportedClockRelationship => m.timeline = 2,
            _ => {}
        }
        p.step(Some((m, 0.5, expected != R::UnknownBinding)), t(140), Some(expected));
        p.accepted_at(Some(t(100)));
        p.step(None, t(151), Some(R::WatchdogExpired));
...
fn renewal_preserves_sequence_and_deadline_and_cannot_revive_expired_lease() {
    ...
    assert_eq!(p.replace(config(7, 1000, 1000)), Err(SessionError::ReusedGeneration));
    assert_eq!(p.replace(config(6, 1000, 1000)), Err(SessionError::ReusedGeneration));
...
fn renewal_does_not_extend_source_age_or_watchdog() {
    for (age, watchdog, edge, reason) in [(100, 1000, 201, R::StaleCommand), (1000, 50, 151, R::WatchdogExpired)] {
...
fn replacement_revocation_and_restart_reject_old_generations() {
    ...
    let mut fresh = meta(0, 103, 500); fresh.generation = Generation::new(8).unwrap();
    p.send(fresh, 103, None);
    p.host.leases_mut().revoke(&Identity::new("controller"), &Capability::new("thrust"));
    p.embedded.revoke_lease();
    p.step(None, t(104), Some(R::LeaseRevoked));
    assert_eq!(p.renew(2000, 105), Err(SessionError::RevokedSession));
    assert_eq!(p.replace(config(8, 100, 50)), Err(SessionError::ReusedGeneration));
    ...
    fresh.generation = Generation::new(9).unwrap(); fresh.source_at = t(105);
...
fn gate_clock_faults_latch_across_session_changes() {
    for (bad, reason) in [(t(99), R::EvaluationTimeRegression), (Timestamp::new(ClockDomain::Simulation, 100), R::EvaluationClockMismatch)] {
        let mut p = Pair::new(config(7, 100, 50));
        p.send(meta(0, 100, 300), 100, None);
        p.step(None, bad, Some(reason));
        p.replace(config(8, 100, 50)).unwrap();
        let mut m = meta(0, 101, 300); m.generation = Generation::new(8).unwrap();
...
fn extreme_timestamp_arithmetic_is_checked_on_both_paths() {
    let wide = SessionConfig::new(Generation::new(7).unwrap(), t(i128::MIN), t(i128::MAX), config(7, 100, 50).policy()).unwrap();
    let mut p = Pair::new(wide);
    p.step(Some((meta(0, i128::MIN, i128::MIN + 100), 0.5, true)), t(i128::MIN), None);
    p.step(None, t(i128::MAX - 1), Some(R::EvaluationTimeOverflow));
    p.step(None, t(i128::MAX - 1), Some(R::EvaluationTimeOverflow));
    let mut p = Pair::new(wide);
    p.step(Some((meta(0, i128::MIN, i128::MAX), 0.5, true)), t(i128::MAX - 1), Some(R::CommandTimeOverflow));
    let mut p = Pair::new(wide);
    p.step(Some((meta(0, 0, i128::MAX), 0.5, true)), t(i128::MIN), Some(R::FutureCommand));
}
...
fn unknown_payload_identities_do_not_allocate_tracking_slots() {
    let mut p = Pair::new(config(7, 100, 50));
    for i in 0..1000 {
        let input = CommandRequest::new(Identity::new(format!("unknown-{i}")), Capability::new("thrust"), 0.5, meta(0, 100, 300));
        assert_eq!(p.host.evaluate(Some(input), t(100)).outcome, Outcome::Rejected(R::UnknownBinding));
    }
