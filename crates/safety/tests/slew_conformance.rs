...
use neuradix_safety::{
    ...
    Identity, LeaseTable, Outcome, RejectReason as R, SafetyGate, SessionConfig, SharedTimeline,
};
use neuradix_time::{ClockDomain, Duration, Timestamp};

fn t(nanos: i128) -> Timestamp {
    Timestamp::new(ClockDomain::Monotonic, nanos)
}
fn ms(value: i128) -> Timestamp {
    t(value * 1_000_000)
}
...
impl Rig {
    fn new(rate: f32, min: f32, max: f32, safe: f32, slew_first: bool) -> Self {
        ...
        let mut constraints = vec![
            Constraint::range("range", min as f64, max as f64).unwrap(),
        ...
        Self {
            ...
            embedded: mcu::CommandGate::new(
                mcu::Limits::with_slew_rate(min, max, rate).unwrap(),
                mcu::AuthorityLease::new(1, 2, config(1)),
                safe,
            )
            .unwrap(),
            rate: rate as f64,
    ...
    fn eval_with_source(
        ...
        let meta = CommandMeta {
            ...
            source_at: source,
            deadline: t(source.as_nanos().saturating_add(10_000_000_000)),
            timeline: 1,
        };
        let host_input = input.map(|value| {
            CommandRequest::new(
                Identity::new("controller"),
                Capability::new("thrust"),
                value as f64,
        ...
        let mcu_input = input.map(|value| mcu::Command {
            holder: 1,
            capability: 2,
            value,
            meta,
        ...
        assert_eq!(
            match e.outcome {
                mcu::Outcome::SafeState(r) => Some(r),
                _ => None,
            },
            expected
        );
        ...
        assert!(
            (values.0 - values.1).abs() <= tolerance,
            "host/MCU {:?} at {now}, tolerance {tolerance}",
            values
        );
        ...
        if expected.is_some() {
            ...
            for (value, prev) in [(values.0, hprev), (values.1, eprev)] {
                if input.is_none() || budget == 0.0 {
                    ...
                    assert!(
                        (value - prev).abs() <= budget + rounding,
                        "rate exceeded: {prev} -> {value}, budget {budget}"
                    );
                }
            }
        }
        if input.is_some() && expected.is_none() {
            self.sequence += 1;
        }
    ...
    fn replace(&mut self, generation: u128) {
        self.host
            ...
            .unwrap();
        self.embedded
            .replace_lease(mcu::AuthorityLease::new(1, 2, config(generation)))
            .unwrap();
        self.generation = Generation::new(generation).unwrap();
        self.sequence = 0;
    }
    fn accepted_at(&self) -> Option<Timestamp> {
        ...
        assert_eq!(at, self.embedded.last_accepted_at());
        at
    }
}
...
fn constant_target_has_equal_progress_over_different_tick_partitions() {
    for periods in [
        ...
        for dt in periods {
            elapsed += dt;
            r.eval(Some(10.0), ms(elapsed), None);
        }
...
fn jitter_zero_periods_and_direction_reversals_obey_rate() {
    for rate in [0.25, 1.0, 25.0] {
        ...
        for (dt, target) in [
            ...
            elapsed += dt;
            r.eval(Some(target), ms(elapsed), None);
        }
    }
}
...
fn first_evaluation_initializes_but_an_idle_first_tick_establishes_safe_reference() {
    ...
    assert!((host - 0.1).abs() < 1e-12);
    assert!((embedded - 0.1).abs() < 1e-6);
}
...
fn rejection_safes_immediately_and_recovery_uses_the_last_safe_tick() {
    ...
    let (host, embedded) = r.eval(Some(1.0), ms(200), None);
    assert!((host + 0.2).abs() < 1e-12);
    assert!((embedded + 0.2).abs() < 1e-6);
}
...
fn renewal_replacement_and_revocation_do_not_reset_slew_reference() {
    ...
    r.host
        ...
        .unwrap();
    r.embedded
        .replace_lease(mcu::AuthorityLease::new(1, 2, short))
        .unwrap();
    r.generation = Generation::new(2).unwrap();
    r.sequence = 0;
    r.host
        .renew_lease(
            &Identity::new("controller"),
    ...
    assert!((host - 0.01).abs() < 1e-12);
    assert!((embedded - 0.01).abs() < 1e-6);
}
...
fn binary32_quantization_can_hold_without_accumulating_fractional_budget() {
    ...
    assert_eq!(
        coarse.eval(Some(tiny), ms(1000), None),
        (tiny as f64, tiny as f64)
    );
}
...
fn extreme_finite_binary32_values_remain_finite_on_both_paths() {
    ...
    assert_eq!(
        r.eval(Some(f32::MAX), ms(2000), None),
...
fn clock_faults_still_latch_after_slew_and_session_replacement() {
    for (bad, reason) in [
        (ms(99), R::EvaluationTimeRegression),
        (
            Timestamp::new(ClockDomain::Utc, 100_000_000),
            R::EvaluationClockMismatch,
        ),
    ] {
        let mut r = Rig::new(1.0, -1.0, 1.0, 0.0, false);
        r.eval(Some(0.5), ms(100), None);
        r.eval(None, bad, Some(reason));
        r.replace(2);
