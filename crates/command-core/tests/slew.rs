...
fn invalid_rates_and_arguments_cannot_produce_outputs() {
    ...
    assert!(rate.apply_f64(1.0, Some((0.0, Duration::from_nanos(-1)))).is_none());
...
fn zero_elapsed_zero_rate_and_initialization_are_explicit() {
    ...
    assert_eq!(SlewRate::new(0.0).unwrap().apply_f64(7.0, Some((2.0, Duration::from_nanos(i128::MAX)))), Some(2.0));
}
...
fn arithmetic_overflow_rejects_instead_of_granting_unlimited_slew() {
    ...
    for prev in [-f64::MAX, 0.0, f64::MAX] {
        assert!(rate.apply_f64(1.0, Some((prev, Duration::from_secs(3)))).is_none());
    }
    assert!(rate.apply_f32(1.0, Some((0.0, Duration::from_secs(3)))).is_none());
    let tiny = SlewRate::new(f64::from_bits(1)).unwrap();
    assert_eq!(tiny.apply_f64(1.0, Some((0.0, Duration::from_nanos(1)))), Some(0.0));
}
...
fn binary32_rounding_is_inward_at_half_ulp_and_subnormal_boundaries() {
    for sign in [-1.0f32, 1.0] {
        let prev = sign;
        let next = if sign > 0.0 { prev.next_up() } else { prev.next_down() };
        let ulp = (next as f64 - prev as f64).abs();
        let rate = SlewRate::new(ulp * 0.75).unwrap();
        assert_eq!(rate.apply_f32(next, Some((prev, Duration::from_secs(1)))), Some(prev));
        let rate = SlewRate::new(ulp).unwrap();
        assert_eq!(rate.apply_f32(next, Some((prev, Duration::from_secs(1)))), Some(next));
    }
    let tiny = f32::from_bits(1);
    let rate = SlewRate::new(tiny as f64 * 0.75).unwrap();
    assert_eq!(rate.apply_f32(tiny, Some((0.0, Duration::from_secs(1)))), Some(0.0));
}
