...
fn main() {
    ...
    raw.spec.as_mut().unwrap().components[0].configuration =
        serde_yaml::from_str("{sample_period_us: 1000}").unwrap();
    ...
    println!(
        "original={}\nchanged={}",
        original.resolved_identity().unwrap(),
        changed.resolved_identity().unwrap()
    );
}
