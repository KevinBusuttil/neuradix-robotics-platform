...
pub(super) fn decode(data: &[u8], expected: usize) -> Result<Vec<u8>> {
    ...
    if c.take(4)? != [0x04, 0x22, 0x4d, 0x18] {
        return Err(RecordError::UnsupportedMcap(
            "LZ4 requires one standard frame",
        ));
    }
    ...
    if flags >> 6 != 1 || flags & 0x03 != 0 || bd & 0x8f != 0 {
        return Err(RecordError::UnsupportedMcap(
            "LZ4 version, reserved bits or dictionary",
        ));
    }
    ...
    loop {
        ...
        if len == 0 || len > block_max {
            return Err(malformed("invalid LZ4 block length"));
        }
        c.take(len)?;
        if flags & 0x10 != 0 {
            c.take(4)?;
    ...
    if c.remaining() != 0 {
        return Err(malformed("trailing LZ4 data"));
    }
