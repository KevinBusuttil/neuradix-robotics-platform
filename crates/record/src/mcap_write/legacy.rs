...
use crate::{
    McapChannel, McapHeader, McapMessageRef, McapMetadata, McapSchema, RecordError,
    RecordingManifest, Result,
};
...
impl<W: Write> McapWriter<W> {
    ...
    pub fn with_limits(
        ...
        policy: McapWriteLimits,
    ) -> Result<Self> {
        if manifest.format_version != crate::FORMAT_VERSION {
            return Err(RecordError::UnsupportedVersion(manifest.format_version));
        }
        if manifest.channels.len() > u16::MAX as usize {
            return Err(RecordError::TooManyChannels(manifest.channels.len()));
        }
        limits::check(
            manifest.channels.len() as u64,
            policy.channels() as u64,
            "channels",
        )?;
        limits::check(
            manifest.channels.len() as u64,
            policy.schemas() as u64,
            "schemas",
        )?;
        let mut counter = Counter {
            bytes: 0,
            limit: policy.string_bytes(),
        };
        serde_json::to_writer(&mut counter, manifest).map_err(|_| RecordError::WriteLimit {
            kind: "manifest JSON bytes",
        ...
        for (index, c) in manifest.channels.iter().enumerate() {
            let domain = ClockDomain::parse(&c.clock_domain).ok_or(
                RecordError::InvalidMcapWrite("unknown manifest clock domain"),
            )?;
            if domains.contains_key(&c.id) {
                return Err(RecordError::InvalidMcapWrite("duplicate manifest channel"));
            }
            // JSON admission bounded all strings and caller clones before these allocations.
            let schema_id = (index + 1) as u16;
            writer.schema(&McapSchema {
                id: schema_id,
                name: c.name.clone(),
                encoding: "neuradix/schema-id".into(),
                data: c.schema_id.as_bytes().to_vec(),
            })?;
            writer.channel(&McapChannel {
                id: c.id,
                schema_id,
                topic: c.name.clone(),
    ...
    pub fn write_record(
        ...
        if self.domains.get(&channel_id) != Some(&timestamp.domain()) {
            return self.writer.reject(RecordError::InvalidMcapWrite(
                "unknown channel or clock-domain mismatch",
            ));
        }
        let sequence = match u32::try_from(sequence) {
            Ok(n) => n,
            Err(_) => return self.writer.reject(RecordError::SequenceTooLarge(sequence)),
        };
        let nanos = timestamp.as_nanos();
        let time = match u64::try_from(nanos) {
            Ok(n) => n,
            Err(_) => return self.writer.reject(RecordError::TimestampOutOfRange(nanos)),
        };
        self.writer.message(McapMessageRef {
            channel_id,
            sequence,
            log_time: time,
            publish_time: time,
...
impl Write for Counter {
    fn write(&mut self, data: &[u8]) -> io::Result<usize> {
        self.bytes = self
            ...
            .ok_or_else(|| io::Error::other("manifest size limit"))?;
        Ok(data.len())
    }
