        ...
        "complete_statistics_and_duplicate_keys" => {
            fn with_stats(records: &[(u8, Vec<u8>)], stats: &[u8]) -> Vec<u8> {
                ...
                for (op, body) in records {
                    append(&mut out, *op, body);
                }
                append(&mut out, 15, &[0; 4]);
                let start = out.len() as u64;
                append(&mut out, 11, stats);
                let mut footer = start.to_le_bytes().to_vec();
                footer.extend_from_slice(&[0; 12]);
                append(&mut out, 2, &footer);
                out.extend_from_slice(&MCAP_MAGIC);
                out
            }
            let original = data_records();
            let stats = parts(&fixture("uncompressed"))
                .into_iter()
                .find(|(op, _)| *op == 11)
                .unwrap()
                .1;
            ...
            missing.truncate(56);
            missing[42..46].copy_from_slice(&10u32.to_le_bytes());
            let error = archive(&with_stats(&original, &missing), limits).unwrap_err();
            assert!(error.to_string().contains("omit a nonzero"), "{error}");
            let mut duplicate = stats.clone();
            duplicate.extend_from_slice(&stats[46..56]);
            duplicate[42..46].copy_from_slice(&30u32.to_le_bytes());
            let error = archive(&with_stats(&original, &duplicate), limits).unwrap_err();
            assert!(error.to_string().contains("Record parse failed"), "{error}");
