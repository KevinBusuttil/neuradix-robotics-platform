...
impl TempDir {
    pub fn new(label: &str) -> Self {
        ...
        let dir = std::env::temp_dir()
            .join(format!("neuradix-{label}-{}-{id}", std::process::id()));
...
pub fn depth() -> Contract {
    load_file(&PathBuf::from(env!("CARGO_MANIFEST_DIR"))
        .join("../../contracts/standard/navigation/vehicle-depth.yaml")).unwrap()
}

pub fn tiny() -> Contract {
    load_file(&PathBuf::from(env!("CARGO_MANIFEST_DIR"))
        .join("tests/fixtures/tiny-telemetry.yaml")).unwrap()
}

pub fn require_compiler(candidates: &[&str]) -> String {
    candidates.iter().find(|cc| {
...
pub fn run_cpp(dir: &TempDir, source: &str) {
    ...
    success(Command::new(compiler)
        .args(["-std=c++11", "-O2", "-Wall", "-Wextra", "-Werror"])
        .arg(&source_path).arg("-o").arg(&bin).output().unwrap());
