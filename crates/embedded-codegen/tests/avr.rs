...
const AVR_FLAGS: &[&str] = &[
    "-mmcu=atmega328p", "-std=gnu++11", "-Os", "-Wall", "-Wextra", "-Werror",
...
fn avr_compiles_and_links_supported_scalars_within_uno_memory() {
    ...
    success(Command::new(compiler).args(AVR_FLAGS).arg(source)
        .arg("-Wl,--gc-sections").arg("-o").arg(&elf).output().unwrap());
    ...
    let sizes: Vec<usize> = output.lines().nth(1).unwrap().split_whitespace()
        .take(3).map(|v| v.parse().unwrap()).collect();
    assert_eq!(sizes.len(), 3);
    assert!(sizes[0] + sizes[1] <= 32_256, "exceeds Uno application flash");
...
fn avr_rejects_portable_binary64_header_with_explicit_diagnostic() {
    ...
    std::fs::write(dir.0.join("vehicle_depth.h"), generate_cpp(&depth()).unwrap().code).unwrap();
    let source = dir.0.join("main.cpp");
    std::fs::write(&source, "#include \"vehicle_depth.h\"\nint main() { return 0; }\n").unwrap();
    let output = Command::new(compiler).args(AVR_FLAGS).arg("-c").arg(source)
        .arg("-o").arg(dir.0.join("main.o")).output().unwrap();
    let diagnostic = String::from_utf8_lossy(&output.stderr);
    assert!(!output.status.success(), "AVR must not compile binary64 memcpy code");
    assert!(diagnostic.contains("Neuradix float64 requires IEEE-754 binary64"),
